...
static int video_open(struct device *dev, const char *devname, int no_query)
{
       ...
       printf("555555%s55555 %d",devname,no_query);
	...
	if (!no_query) {
		...
		printf("###video_open##ioctl(dev->fd, VIDIOC_QUERYCAP, &cap); \n");
...
static void uvc_get_control(struct device *dev, unsigned int id)
{
	...
	ctrl.id = id;
		printf("##uvc_get_control### ioctl(dev->fd, VIDIOC_G_CTRL, &ctrl); \n");
...
static void uvc_set_control(struct device *dev, unsigned int id, int value)
{
	...
	ctrl.id = id;
	ctrl.value = value;
		printf("###uvc_set_control## ioctl(dev->fd, VIDIOC_S_CTRL, &ctrl); \n");
...
static int video_get_format(struct device *dev)
{
	...
	fmt.type = dev->type;
		printf("##video_get_format### ioctl(dev->fd, VIDIOC_G_FMT, &fmt); \n");
...
static int video_set_format(struct device *dev, unsigned int w, unsigned int h, unsigned int format)
{
	...
	fmt.fmt.pix.field = V4L2_FIELD_ANY;
		printf("##video_set_format### ioctl(dev->fd, VIDIOC_S_FMT, &fmt); \n");
...
static int video_set_framerate(struct device *dev, struct v4l2_fract *time_per_frame)
{
	...
	parm.type = dev->type;
		printf("##video_set_framerates### ioctl(dev->fd, VIDIOC_G_PARM, &parm); \n");
	...
	parm.parm.capture.timeperframe.denominator = time_per_frame->denominator;
		printf("##video_set_framerate### ioctl(dev->fd, VIDIOC_S_PARM, &parm); \n");
...
static int video_alloc_buffers(struct device *dev, int nbufs, unsigned int offset)
{
	...
	rb.memory = dev->memtype;
		printf("##video_alloc_buffers### ioctl(dev->fd, VIDIOC_REQBUFS, &rb); \n");
	...
	for (i = 0; i < rb.count; ++i) {
		...
		buf.memory = dev->memtype;
			printf("###video_alloc_buffers## ioctl(dev->fd, VIDIOC_QUERYBUF, &buf); \n");		
...
static int video_free_buffers(struct device *dev)
{
	...
	rb.type = dev->type;
	rb.memory = dev->memtype;
			printf("##video_free_buffers### ioctl(dev->fd, VIDIOC_REQBUFS, &rb); \n");		
	ret = ioctl(dev->fd, VIDIOC_REQBUFS, &rb);
	if (ret < 0) {
...
static int video_queue_buffer(struct device *dev, int index)
{
	...
	if (dev->type == V4L2_BUF_TYPE_VIDEO_OUTPUT) {
		...
		memset(dev->buffers[buf.index].mem, 0x55, buf.length);
			printf("##video_queue_buffer### ioctl(dev->fd, VIDIOC_QBUF, &buf); \n");		
...
static int video_enable(struct device *dev, int enable)
{
	int type = dev->type;
	int ret;
			printf("###video_enable## ioctl(dev->fd, enable ? VIDIOC_STREAMON : VIDIOC_STREAMOFF, &type); \n");		
...
static void video_query_menu(struct device *dev, unsigned int id)
{
	...
	while (1) {
		menu.id = id;
					printf("##video_query_menu### ioctl(dev->fd, VIDIOC_QUERYMENU, &menu); \n");		
...
static void video_list_controls(struct device *dev)
{
	...
	while (1) {
...
#endif
					printf("##video_list_controls### ioctl(dev->fd, VIDIOC_QUERYCTRL, &query); \n");		
		...
		ctrl.id = query.id;
					printf("##video_list_controlss###  ioctl(dev->fd, VIDIOC_G_CTRL, &ctrl); \n");		
		ret = ioctl(dev->fd, VIDIOC_G_CTRL, &ctrl);
		if (ret < 0)
			strcpy(value, "n/a");
		else
	...
	unsigned int width, unsigned int height)
{
	...
	for (i = 0; ; ++i) {
		...
		ival.width = width;
		ival.height = height;
				printf("#####  ioctl(dev->fd, VIDIOC_ENUM_FRAMEINTERVALS, &ival); \n");		
...
static void video_enum_frame_sizes(struct device *dev, __u32 pixelformat)
{
	...
	for (i = 0; ; ++i) {
		...
		frame.pixel_format = pixelformat;
				printf("##video_enum_frame_sizes###  ioctl(dev->fd, VIDIOC_ENUM_FRAMESIZES, &frame); \n");				
...
static void video_enum_formats(struct device *dev, enum v4l2_buf_type type)
{
	...
	for (i = 0; ; ++i) {
		...
		fmt.index = i;
		fmt.type = type;
				printf("###video_enum_formats##  ioctl(dev->fd, VIDIOC_ENUM_FMT, &fmt); \n");				
...
static void video_enum_inputs(struct device *dev)
{
	...
	for (i = 0; ; ++i) {
		...
		input.index = i;
				printf("##video_enum_inputs### ioctl(dev->fd, VIDIOC_ENUMINPUT, &input);  \n");				
...
static int video_get_input(struct device *dev)
{
	__u32 input;
	int ret;
				printf("##video_get_input### ioctl(dev->fd, VIDIOC_G_INPUT, &input); \n");				
...
static int video_set_input(struct device *dev, unsigned int input)
{
	__u32 _input = input;
	int ret;
				printf("##video_set_input### ioctl(dev->fd, VIDIOC_S_INPUT, &_input); \n");				
...
static int video_set_quality(struct device *dev, unsigned int quality)
{
	...
	jpeg.quality = quality;
				printf("##video_set_quality### ioctl(dev->fd, VIDIOC_S_JPEGCOMP, &jpeg); \n");				
	...
	if (ret < 0) {
		...
		return ret;
				...
				printf("##video_set_quality### ioctl(dev->fd, VIDIOC_G_JPEGCOMP, &jpeg); \n");				
	...
	int do_requeue_last)
{
	...
	for (i = 0; i < nframes; ++i) {
		...
		buf.type = dev->type;
		buf.memory = dev->memtype;
printf("##video_do_capture### ioctl(dev->fd, VIDIOC_DQBUF, &buf); \n");
